...
def plot_roc_curve(pdf: PdfPages, predictions: pd.DataFrame) -> None:
    ...
    """
    fpr, tpr, _ = roc_curve(predictions.y_true, predictions.y_probs) #most likely needs to be adapted for multiclass
    plt.figure()
...
def plot_confusion_matrices(
    ...
    plt.figure(figsize=(8, 8))
    ConfusionMatrixDisplay(confusion_matrix=weighted_cm, display_labels=classes).plot()
...
def modelPerformance(
    ...
    # fpr_measure = fp / (fp + tn) # used in binary classification, should be adapted if wanted in multiclass classification
